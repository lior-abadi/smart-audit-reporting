...
contract Emitter {
    ...
    struct Amounts {
        uint256 totalAmt; //@SAR-G iter-incremental
        uint256 tokenAmt; //@SAR-L indexed-events
        uint256 refererrAmt; //@SAR-G iter-incremental
    }
    ...
    constructor(address _golomTrader) {
        eip712DomainHash = keccak256(
            abi.encode(
                ...
                keccak256(bytes("1")),
                1, //@SAR-L two-step-process
                _golomTrader //@SAR-G iter-incremental
            )
        );
    }
    ...
    function hashPayment(Payment calldata p) private pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "payment(uint256 paymentAmt,address paymentAddress)"
                    ),
                    p.paymentAmt,
                    p.paymentAddress
                )
    ...
    function _hashOrderinternal(Order calldata o, uint256[2] memory extra)
        private
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    ...
                    o.refererrAmt,
                    o.root,
                    o.reservedAddress, //@SAR-L indexed-events
                    extra
                )
            );
    }
